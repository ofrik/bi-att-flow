...
from basic.evaluator2 import F1Evaluator, Evaluator, ForwardEvaluator, MultiGPUF1Evaluator
from basic.graph_handler import GraphHandler
from basic.model2 import Model, get_multi_gpu_models
from basic.trainer import Trainer, MultiGPUTrainer

from basic.main import set_dirs
from basic.read_data import load_metadata, read_data2, get_squad_data_filter, update_config
...
class Demo(object):
    ...
    def data_ready(self, data=None, update=False):
        ...
        config = self.config
        config.batch_size = 1
        test_data = read_data2(self.config, 'demo', True, data=data, data_set=self.test_data)
    ...
    def run(self, data):
        ...
        config = self.config
        config.max_ques_size = len(data["q"][0])
        e = None
